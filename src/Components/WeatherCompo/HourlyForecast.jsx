...
const HourlyForecast = () => {
  ...
  return (
    <div className="min-md:w-4/5 p-3 bg-[#ccc] shadow-lg/30 flex flex-col items-center rounded-2xl mt-5 min-md:mt-0">
      ...
      <div className="min-md:flex w-full justify-around gap-5 mt-5 space-y-5">
          ...
          return (
            ...
            >
              ...
              className="w-14 m-auto my-2"
                src={item.weather[0].main == "Clear" ? sunny :
                  item.weather[0].main == "Clouds" ? partcloudy :
                  item.weather[0].main == "Rain" ? rainy :
                  item.weather[0].main == "Thunderstorm" ? rainythunder :
                  item.weather[0].main == "Snow" ? snowy : {}
                }
                alt=""
              />
              <h3 className="text-xl font-bold text-white">{Math.trunc(item.main.temp - 273.15)}°C</h3>
            </div>
          );
        })}
      </div>
